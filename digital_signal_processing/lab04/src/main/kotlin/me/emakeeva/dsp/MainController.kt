...
import me.emakeeva.dsp.operators.Operators
...
class MainController : Controller() {
    ...
    fun convert(link: String, operator: Operators, prop: SimpleDoubleProperty) {
        launch(prop) {
            val image = urlToBytes(link)
            val gray = image.toGray()
            val res = operator.callback.invoke(gray).toByteArr()
